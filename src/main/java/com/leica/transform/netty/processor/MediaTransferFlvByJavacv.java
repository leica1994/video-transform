...
public class MediaTransferFlvByJavacv implements Runnable {
    ...
    public MediaTransferFlvByJavacv(HttpRequestWrapper requestWrapper) {
    ...
    protected boolean createGrabber() {
        // 拉流器
        grabber = new FFmpegFrameGrabber(requestWrapper.getUrl());
        // 超时时间(15秒)
...
//		grabber.setAudioStream(1);
        ...
        try {
            grabber.start();
            log.info("\r\n{}\r\n启动拉流器成功", requestWrapper.getUrl());
            return grabberStatus = true;
        } catch (Exception e) {
            log.error("\r\n{}\r\n启动拉流器失败，网络超时或视频源不可用", requestWrapper.getUrl());
//			e.printStackTrace();
        }
...
//			recorder.setAudioCodecName("aac");
            ...
            try {
                ...
                log.warn("\r\n{}\r\n启动转复用录制器失败", requestWrapper.getUrl());
                // 如果转复用失败，则自动切换到转码模式
                transferFlag = false;
                if (recorder != null) {
                    try {
                ...
                if (createTransterOrRecodeRecorder()) {
                    log.error("\r\n{}\r\n切换到转码模式", requestWrapper.getUrl());
                    return true;
                }
                log.error("\r\n{}\r\n切换转码模式失败", requestWrapper.getUrl());
                e.printStackTrace();
            }
        }
...
//						continue;
        ...
        log.info("关闭媒体流-javacv，{} ", requestWrapper.getUrl());
    }
    ...
    public void hasClient() {
        ...
        if (hcSize != newHcSize || wcSize != newWcSize) {
            hcSize = newHcSize;
            wcSize = newWcSize;
            log.info("\r\n{}\r\nhttp连接数：{}, ws连接数：{} \r\n", requestWrapper.getUrl(), newHcSize, newWcSize);
        }


        if (httpClients.isEmpty() && wsClients.isEmpty()) {
            // 等待20秒还没有客户端，则关闭推流
            if (noClient > 20000000) {
                closeMedia();
            } else {
                noClient += 1000;
